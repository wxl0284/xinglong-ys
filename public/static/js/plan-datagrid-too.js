//观测计划的赤经和赤纬 js事件//////////////////////////////
	...
	planInfo.on('keyup', 'td[field="rightAscension1"] input.textbox-text', function () {//赤经之小时 js事件
		let that = $(this);
		let parent_td_rightAscension1 = that.closest('td[field="rightAscension1"]'); //当前input的父元素:td[field="rightAscension1"]
		let td_rightAscension2 = parent_td_rightAscension1.siblings('td[field="rightAscension2"]'); //同辈的：td[field="rightAscension2"]
		let patn = /^\d{2}$/;
		let v = $.trim(that.val());
	...
	planInfo.on('keyup', 'td[field="rightAscension2"] input.textbox-text', function () {
		...
		let patn = /^\d{2}$/;
		let v = $.trim(that.val());
		let parent_td_rightAscension2 = that.closest('td[field="rightAscension2"]'); //当前input的父元素:td[field="rightAscension2"]
		let td_rightAscension3 = parent_td_rightAscension2.siblings('td[field="rightAscension3"]'); //同辈的：td[field="rightAscension3"]
	...
	planInfo.on('keyup', 'td[field="declination1"] input.textbox-text', function () {//赤纬之小时 js事件
		...
		let v = $.trim(that.val());
		let patn = /^-?\d{1,2}$/;
		let v_R = v.replace(/-/, ''); //将-替换为空字符

		let parent_td_declination1 = that.closest('td[field="declination1"]'); //当前input的父元素:td[field="declination1"]
		let td_declination2 = parent_td_declination1.siblings('td[field="declination2"]'); //同辈的：td[field="declination2"]
	...
	planInfo.on('keyup', 'td[field="declination2"] input.textbox-text', function () {//赤纬之分钟 js事件
		...
		let patn = /^\d{2}$/;
		let v = $.trim(that.val());
		let parent_td_declination2 = that.closest('td[field="declination2"]'); //当前input的父元素:td[field="declination2"]
		let td_declination3 = parent_td_declination2.siblings('td[field="declination3"]'); //同辈的：td[field="declination3"]
...
//导入计划文件 上传////////////////////////////////////////////
	...
	{	
		let rowIndex = getrow(target); //当前行索引 自定义的函数
		table.datagrid('endEdit', editRow);
