...
class Page extends Base
{
    ...
    public function config()
    {
        ...
        */
        ...
        $errMsg = '';  //错误提示
        if ( $confNum < 16 )    //不够16个时
        {//逐一判断缺少了哪个固定属性
            if ( !in_array(['conf' => 'gimbaltype'], $confOption) )
            {//缺少了'焦点类型'
                $errMsg .= '固定属性还须添加：转台类型!<br>';
            }
        ...
        }/*检查判断数据表'confoption'内16个动态增减的固定属性  结束*/
        ...
        {//还须添加固定属性
            ...
            /*1、获取动态增减的固定属性数据*/
            $result['confOption'] = $this->get_16confOption ();            
            /*获取动态增减的固定属性数据 结束*/
    ...
    */
    ...
    {
        $dir = config('at_pic_dir'); //各望远镜png图片路径
    ...
    public function down_fits_pic () //下载单个fits图片
    {
        ...
        $file = $this->path . $postData['file_name'];
    ...
    public function down_multi_fit () //ajax 一次下载多个fit图片
    {
        ...
        if ( $res == true )
        {
            ...
            //$path = $this->path . $postData['files']; //已经
